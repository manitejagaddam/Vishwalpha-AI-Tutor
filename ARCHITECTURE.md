...
1. **Metrics Update (`db/metrics.py`)**:
...
2. **Skill Mapping (`db/metrics.py`)**: The 10 updated raw scores are mathematically aggregated into 5 core **Cognitive Skills** (e.g., Concept Understanding, Learning Adaptability).

### Step 3: Intent Classification

1. **Classifier (`tutor/chat.py`)**: A fast LLM call determines if the question is `"conversational"` (e.g., "Yes I understand") or `"curriculum"` (e.g., "What is an acid?").

### Step 4: Retrieval (If Curriculum)

1. **Semantic Routing (`routing/router.py`)**: The question is parsed to extract metadata constraints (e.g., Chapter: "Acids, Bases and Salts").
2. **Vector Search (`retrieval/engine.py`)**: The system embeds the question and queries Qdrant.
3. **Confidence Gating**: The retrieved chunks are filtered. If no chunk has a cosine similarity score above `0.60`, the system blocks the LLM and returns a polite refusal to prevent hallucinations.
4. **Compression (`retrieval/reranker.py`)**: Valid chunks are combined into a `context` string.

### Step 5: Personalized Generation

1. **Prompt Construction (`tutor/llm.py`)**: `TutorLLM` builds a massive system prompt. It injects:
...
2. **LLM Inference**: The prompt is sent to Groq (Llama 3), which streams back the personalized educational response.

### Step 6: Persistence & Display

1. **Database Save (`db/memory.py`)**: The student's question and the tutor's answer are saved as a new `Turn` in PostgreSQL.
2. **UI Render (`streamlit_app.py`)**: The response is displayed to the user. Simultaneously, the right-hand panel updates to show the retrieved chunks, the exact prompt sent, and the real-time cognitive score adjustments.
